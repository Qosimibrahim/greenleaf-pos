...
export async function seedDB() {
  try {
    ...
    if (taxCount === 0) {
      ...
      await TaxSettings.create({ rate: 0.075, currency: "NGN", exchange_rate: 1, currency_symbol: "₦" });
    }
    ...
    if (catCount === 0) {
      ...
      await ProductCategory.create({ name: "Accessories" });
    }
    ...
    // 7. Ensure Default Admin & Cashier Users Always Exist
    const adminUser = await User.findOne({ email: "admin@clientapp.demo" });
    if (!adminUser) {
      console.log("Seeding Admin User (admin@clientapp.demo)...");
    ...
    const cashierUser = await User.findOne({ email: "cashier@clientapp.demo" });
    if (!cashierUser) {
      console.log("Seeding Cashier User (cashier@clientapp.demo)...");
